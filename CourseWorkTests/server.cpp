...
void Server::run(int port, const std::string& databaseFile, const std::string& logFile)
{
    ...
    if (authenticateClient(clientUsername, clientPassword, databaseFile)) {
        ...
        // Пример обработки векторов от клиента
        int numVectors;
...
{
    // Генерация случайного salt, например, 16 шестнадцатеричных символов
    // Замените этот код на более надежный метод генерации случайных значений
    std::string salt;
    for (int i = 0; i < 16; ++i) {
